...
class Linear(nn.Module, LoraLayer):
    ...
    def update_moe_layer(
        ...
        self.lora_dropout.update(nn.ModuleDict({adapter_name: lora_dropout_layer}))
        # Actual trainable parameters
        # experts in dict, 初始化 num_experts 个 LoRA 模块
        for i in range(num_experts):
    ...
    def moe_forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        moe_type="label_wise" # "token_wise"

        # import traceback
        # traceback.print_stack(limit=100)
        
        # Edited by wangzh for label_wise moe
        ...
        if self.disable_adapters:
            ...
            if moe_type == "token_wise":
                ...
                bsz = x.shape[0]
                route_weight = torch.zeros((bsz, self.num_experts), device=x.device, dtype=result.dtype)

                if isinstance(labels, list) or isinstance(labels, torch.Tensor):
                    if isinstance(labels, list):
                        labels = torch.tensor(labels, device=x.device)
                    route_weight[torch.arange(bsz, device=x.device), labels] = 1.0

                route_weight = route_weight.unsqueeze(1).expand(-1, x.size(1), -1)
                route_weight = torch.softmax(route_weight, dim=-1)
                
                print('Testing with label-wise routing!')
                print('activate_adapter_name:', activate_adapter_name, 'route_weight shape:', route_weight.shape)
                ...
                # route_weight.shape : (B, 3072, num_experts), top_k_probs.shape, top_k_indices.shape: (B, 3072, top_k)

                print('Now we are debugging original moe_forward')
                print('args:', args, 'kwargs:', kwargs, 'labels:', labels)
                print('activate_adapter_name:', activate_adapter_name, 'route_logits shape:', route_logits.shape)
                print('top_k_indices shape:', top_k_indices.shape, 'top_k_probs shape:', top_k_probs.shape, 'route_weight shape:', route_weight.shape)
                import pdb
                pdb.set_trace()
                ...
                if labels is not None:
                    ...
                    if isinstance(labels, list):
                        ...
                        if all(isinstance(l, torch.Tensor) for l in labels):
                            # 例如 labels = [tensor([0]), tensor([1])] 或 [tensor([0, 1])]
                            labels = [l.tolist() for l in labels]
                        # list[int] → wrapped with [[..]]
                        if all(isinstance(l, int) for l in labels):
                            labels = [labels]
                    elif isinstance(labels, torch.Tensor):
                        labels = labels.tolist()
                        if isinstance(labels[0], int):
                            labels = [labels]  # one dim -> two dim

                    # 填充 route_weight
                    for i in range(bsz):
                        if i < len(labels):
                            for lbl in labels[i]:
                                route_weight[i, lbl] = 1.0
                    ...
                    for i in range(self.num_experts):
                        ...
                        scaling = self.scaling[expert_name]
                        dropout = self.lora_dropout[expert_name]
                        result += lora_B(lora_A(dropout(x))) * scaling * route_weight[:, :, i].unsqueeze(-1)
                    ...
                    # 获取 top-k，保持梯度流 ; 计算 softmax，topk之外的weight应该是0 
                    top_k_probs, top_k_indices = torch.topk(route_logits, k=self.top_k, dim=-1)
                    top_k_probs = F.softmax(top_k_probs, dim=-1, dtype=torch.float32).to(result.dtype)
